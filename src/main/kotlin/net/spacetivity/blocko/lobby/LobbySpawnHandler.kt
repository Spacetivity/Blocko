...
class LobbySpawnHandler {
    ...
    init {
        transaction {
            val resultRow = LobbySpawnDAO.selectAll().firstOrNull() ?: return@transaction
    ...
    fun setLobbySpawn(location: Location) {
        ...
        transaction {
            LobbySpawnDAO.insert { statement ->
    ...
    fun updateLobbySpawn(newLocation: Location) {
        transaction {
            LobbySpawnDAO.update({ LobbySpawnDAO.x eq newLocation.x }) { statement ->
